            ...
            <div class="relative p-4 flex flex-col min-w-0 mb-6 break-words bg-white border-0 shadow-soft-xl rounded-2xl bg-clip-border py-4">
                <div class="flex  justify-between align-middle">
                    <div class="p-4 pb-0 mb-0 bg-white rounded-t-2xl">
                        <h6 class="mb-4">Buat Program</h6>
                    </div>
                </div>

                <div>
                    <form role="form text-left" action="{{route('mitra.program.store')}}" method="POST" enctype="multipart/form-data">
                        @csrf

                        <div class="flex gap-4 mb-3">
                            <div class="flex flex-col w-1/2">
                                <span class="text-sm font-medium text-slate-700 mb-2">Judul</span>
                                <input type="text" name="title" class="text-sm focus:shadow-soft-primary-outline leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding py-2 px-3 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:bg-white focus:text-gray-700 focus:outline-none focus:transition-shadow" placeholder="judul..." aria-label="Title" aria-describedby="title-addon" value="{{old('title')}}" />
                                @error('title')
                        ...
                        <div class="flex gap-4 mb-3">
                         ...
                         <div class="flex gap-4 mb-2">
                            <div class="flex flex-col w-1/3">
                                ...
                                <input type="number" name="duration" class="text-sm focus:shadow-soft-primary-outline leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding py-2 px-3 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:bg-white focus:text-gray-700 focus:outline-none focus:transition-shadow" placeholder="durasi..." aria-label="duration" aria-describedby="duration-addon" value="{{old('duration')}}" />
                                @error('duration')
                                <p class="font-semibold text-xs text-red-700">{{$message}}</p>
                                @enderror
                            </div>
                            ...
                            <div class="flex flex-col w-1/3">
                                ...
                                <input type="date" name="end" class="text-sm focus:shadow-soft-primary-outline leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding py-2 px-3 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:bg-white focus:text-gray-700 focus:outline-none focus:transition-shadow" placeholder="tanggal berakhir..." aria-label="end" aria-describedby="end-addon" value="{{old('end')}}" />
                                @error('end')
                                <p class="font-semibold text-xs text-red-700">{{$message}}</p>
                                @enderror
                            </div>
